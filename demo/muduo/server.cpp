...
class DictServer {
    public:
    DictServer(int port): _server(&_evenloop, muduo::net::InetAddress(port),"DictServer",muduo::net::TcpServer::kReusePort)
    {
        //设置连接事件（连接建立/管理）的回调
    ...
    void onMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buf, muduo::Timestamp)
    {
        ...
        std::string msg = buf->retrieveAllAsString(); // 从缓存获取信息
        std::string res;
